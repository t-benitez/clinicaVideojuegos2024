...
var barrel_scene = preload("res://scenes/log.tscn")
...
var last_obs
var Bunny_Start_pos : Vector2i
var Camera_Start_pos : Vector2i

# Called when the node enters the scene tree for the first time.
func _ready():
	$GameOver.hide()
	Bunny_Start_pos = $Bunny.position
...
func new_game():
	...
	#reset the nodes
	$Bunny.position = Bunny_Start_pos
...
func generate_obs():
	...
	if obstacles.is_empty() or last_obs.position.x < score + randi_range(300, 600):
		...
		for i in range(randi() % max_obs + 1):
			...
			var obs_y : int = Bunny_Start_pos.y + 80
			last_obs = obs
		...
		if difficulty == MAX_DIFFICULTY:
			if (randi() % 2) == 0:
				...
				var obs_y : int = bird_heights[randi() % bird_heights.size()] + Bunny_Start_pos.y / 3
				add_obs(obs, obs_x, obs_y)
...
func game_over():
	...
	if (score >= high_score):
		$GameOver.get_node("NewHighScore").visible = true
		$GameOver.get_node("GameOverScreen").visible = false
	else:
		$GameOver.get_node("NewHighScore").visible = false
		$GameOver.get_node("GameOverScreen").visible = true
	$HUD.hide()
	$GameOver.show()
